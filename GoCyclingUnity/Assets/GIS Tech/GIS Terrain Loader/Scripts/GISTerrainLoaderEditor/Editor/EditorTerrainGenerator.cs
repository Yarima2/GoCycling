            ...
            if (ShowTexturePref)
            {
                ...
                switch (textureMode)
                {
                    ...
                    case TextureMode.Splatmapping:
                        ...
                        BaseTerrainLayers.ShowHeight = true;
                        EditorGUILayout.BeginHorizontal();

                        EditorGUILayout.BeginVertical();
                        EditorGUILayout.BeginHorizontal();
                        BaseTerrainLayers.Diffuse = TextureField("", BaseTerrainLayers.Diffuse);
                        BaseTerrainLayers.NormalMap = TextureField("", BaseTerrainLayers.NormalMap);
                        GUILayout.Label(new GUIContent(" Size ", "    "), GUILayout.MaxWidth(200));
                        BaseTerrainLayers.TextureSize = EditorGUILayout.Vector2Field("", BaseTerrainLayers.TextureSize);
                        EditorGUILayout.EndHorizontal();



                        if (BaseTerrainLayers.ShowHeight)
                        {
                            ...
                            BaseTerrainLayers.X_Height = EditorGUILayout.Slider(BaseTerrainLayers.X_Height, 0.0f, 1, GUILayout.ExpandWidth(true));
                            EditorGUILayout.EndHorizontal();
                            EditorGUILayout.BeginHorizontal();
                            GUILayout.Label(" To   ", GUILayout.MaxWidth(50));
                            BaseTerrainLayers.Y_Height = EditorGUILayout.Slider(BaseTerrainLayers.Y_Height, 0.0f, 1, GUILayout.ExpandWidth(true));
                        ...
                        if (ShowSplatmapsTerrainLayer)
                        {
                            ...
                            if (TerrainLayers == null) TerrainLayers = new List<GISTerrainLoaderTerrainLayer>();

                            if (GUILayout.Button(new GUIContent("+", "Add Terrain Layer"), EditorStyles.toolbarButton, GUILayout.ExpandWidth(false)))
                            {
                                TerrainLayers.Add(new GISTerrainLoaderTerrainLayer());
                            }

                            GUILayout.Label("Layers");

                            if (GUILayout.Button("Clear", EditorStyles.toolbarButton, GUILayout.ExpandWidth(false))) TerrainLayers = new List<GISTerrainLoaderTerrainLayer>();
                            ...
                            foreach (GISTerrainLoaderTerrainLayer Tlayer in TerrainLayers)
                            {
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.BeginVertical();
                            ...
                            if (LayerDeleteIndex != -1) TerrainLayers.RemoveAt(LayerDeleteIndex);
                    ...
                    case ".las":
                        {
                            ...
                            if (lasReader.LoadComplet)
                            {
                                ...
                                if (File.Exists(TerrainFilePath))
                                {
                                    ...
                                    if (readingMode == ReadingMode.Full)
                                        TiffReader.LoadTiff(TerrainFilePath, terrainDimensionMode, ElevationInfo.data, TerrainFixOption,EPSGCode);
                    ...
                    case VectorType.ShapeFile:
                        ...
                        if (shapes != null && shapes.Count > 0)
                        {
                            ...
                            foreach (var shape in shapes)
                            {
                                ...
                                if (EnableGeoPointGeneration == OptionEnabDisab.Enable)
                                {
                                    shapeloader.GetGeoVectorPointsData(GeoData);
                                    GISTerrainLoaderGeoPointGenerator.GenerateGeoPoint(GeneratedContainer, GeoData, GeoPointsPrefab);
                                }
