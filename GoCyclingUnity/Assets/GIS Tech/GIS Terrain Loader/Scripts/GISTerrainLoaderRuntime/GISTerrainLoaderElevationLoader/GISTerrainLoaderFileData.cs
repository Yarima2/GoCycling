...
namespace GISTech.GISTerrainLoader
{
    ...
    public class GISTerrainLoaderFileData
    {
        ...
        public Vector2Int GetRowCol(DVector2 LatLon)
        {
            var rang_x = Math.Abs(Math.Abs(DownRightPoint.x) - Math.Abs(TopLeftPoint.x));
            var rang_y = Math.Abs(Math.Abs(TopLeftPoint.y) - Math.Abs(DownRightPoint.y));
            ...
            if (localLat > mapSize_col_x - 1) localLat = mapSize_col_x - 1;
            if (localLon > mapSize_row_y - 1) localLon = mapSize_row_y - 1;

            return new Vector2Int(localLon, localLat);
        }
    }
}