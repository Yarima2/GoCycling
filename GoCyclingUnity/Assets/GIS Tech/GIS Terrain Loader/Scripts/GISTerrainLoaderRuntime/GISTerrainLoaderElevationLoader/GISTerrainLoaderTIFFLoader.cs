...
namespace GISTech.GISTerrainLoader
{
    public class GISTerrainLoaderTIFFLoader
    {
        ...
        private void ParseTiff(Tiff tiff, FixOption fixOption, TerrainDimensionsMode terrainDimensionMode, GISTerrainLoaderFileData LasData)
        {
                    ...
                    switch (BITSPERSAMPLE)
                    {
                        ...
                        case 32:
                            ...
                            else
                            {
                                ...
                                float[] scanline32Bit = new float[tiff.ScanlineSize() / 4];
                                        ...
                                        for (int Col = 0; Col < width; Col += tileWidth)
                                        {
                                            ...
                                            for (int i = 0; i < tileWidth; i++)
                                            {
                                                for (int j = 0; j < tileHeight; j++)
                                                {
                                                    ...
                                                    //short el = BitConverter.ToInt16(Tilebuffer, startIndex);
                                                    var el = (short)((Tilebuffer[startIndex]) + Tilebuffer[startIndex]);
                                                    var el1 = Convert.ToSingle(el) / 2;
                                                    //if (Row ==0) UnityEngine.Debug.Log(pixelX +" " + pixelY + "  " + el1);

                                                    TiffData.BandsData[0][pixelY, pixelX] = el1;

                                                    //GetElevationData(el, pixelX, data.mapSize_row_y - pixelY - 1, fixOption, data);
                ...
                if (LasData == null)
                {
                    try
                    {
                        ...
                        else if (modelTransformationTag != null)
                        {
                            ...
                            TL_X = BitConverter.ToDouble(modelTransformationBytes, 3 * sizeof(double));
                            TL_Y = BitConverter.ToDouble(modelTransformationBytes, (4 + 3) * sizeof(double));

                            DR_X = TL_X + (width * pixelSizeX);
                            DR_Y = TL_Y + (lenght * pixelSizeY);
                        ...
                        if (EPSG == 0)
                        {
                            ...
                            data.TopLeftPoint = new DVector2(TL_X, TL_Y);
                            data.DownRightPoint = new DVector2(DR_X, DR_Y);

                            data.DownLeftPoint = new DVector2(TL_X, data.DownRightPoint.y);
                            data.TopRightPoint = new DVector2(data.DownRightPoint.x, data.TopLeftPoint.y);
            ...
            if (LasData.AlreadyLoaded)
                {
                     CoordinateReferenceSystem = new GTLGeographicCoordinateSystem();

                    if (EPSG == 0)
                    {
                        EPSG = CoordinateReferenceSystem.EPSG_Code = 4326;
                        CoordinateReferenceSystem.GEOGCSProjection = "GCS_WGS_1984";
                    }else
                    {
                        CoordinateReferenceSystem.GEOGCSProjection = "Undefined";

                        data.DownLeftPoint = GeoRefConversion.ConvertTOLatLon(CoordinateReferenceSystem, LasData.DownLeftPoint, EPSG);
                        data.TopRightPoint = GeoRefConversion.ConvertTOLatLon(CoordinateReferenceSystem, LasData.TopRightPoint, EPSG);
                        data.TopLeftPoint = GeoRefConversion.ConvertTOLatLon(CoordinateReferenceSystem, LasData.TopLeftPoint, EPSG);
                        data.DownRightPoint = GeoRefConversion.ConvertTOLatLon(CoordinateReferenceSystem, LasData.DownRightPoint, EPSG);
