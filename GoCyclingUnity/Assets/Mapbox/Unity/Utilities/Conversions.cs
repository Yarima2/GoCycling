...
namespace Mapbox.Unity.Utilities
{
	...
	public static class Conversions
	{
		...
		public static Vector2d StringToLatLon(string s)
		{
			...
			if (latLonSplit.Length != 2 && latLonSplit.Length != 4)
			{
			...
			{
				string lat = latLonSplit[0] + "." + latLonSplit[1];
                if (!double.TryParse(lat, NumberStyles.Any, NumberFormatInfo.InvariantInfo, out latitude))
                {
                    throw new Exception(string.Format("Could not convert latitude to double: {0}", lat));
                }
                string lon = latLonSplit[2] + "." + latLonSplit[3];
                if (!double.TryParse(lon, NumberStyles.Any, NumberFormatInfo.InvariantInfo, out longitude))
                {
                    throw new Exception(string.Format("Could not convert longitude to double: {0}", lon));
                }
            }
